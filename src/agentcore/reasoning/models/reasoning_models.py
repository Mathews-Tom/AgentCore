...
class ReasoningRequest(BaseModel):
    ...
    strategy_config: dict[str, Any] | None = Field(
        default=None, description="Strategy-specific configuration parameters"
    )
...
class ReasoningMetrics(BaseModel):
    ...
    total_tokens: int = Field(
        ..., ge=0, description="Total tokens processed during reasoning"
    )
...
class ReasoningResult(BaseModel):
    ...
    answer: str = Field(..., description="Final answer produced by reasoning process")
...
class CarryoverContent(BaseModel):
    ...
    key_findings: list[str] = Field(
        default_factory=list, description="Important insights discovered so far"
    )
    progress: str = Field(
        ..., description="What has been accomplished in reasoning so far"
    )
    next_steps: list[str] = Field(
        default_factory=list, description="What needs to be done next"
    )
    unresolved: list[str] = Field(
        default_factory=list, description="Open questions or challenges that remain"
    )
...
class BoundedContextIterationResult(BaseModel):
    ...
    answer: str | None = Field(
        default=None, description="Extracted answer if has_answer=True"
    )
    carryover: CarryoverContent | None = Field(
        default=None,
        description="Generated carryover for next iteration (if continuing)",
    )
    metrics: IterationMetrics = Field(..., description="Metrics for this iteration")
...
class BoundedContextResult(BaseModel):
    ...
    iterations: list[BoundedContextIterationResult] = Field(
        ..., min_length=1, description="All iterations executed during reasoning"
    )
    total_tokens: int = Field(
        ..., ge=0, description="Total tokens consumed across all iterations"
    )
    total_iterations: int = Field(
        ..., ge=1, description="Number of iterations executed"
    )
    ...
    carryover_compressions: int = Field(
        default=0, ge=0, description="Number of carryover compressions performed"
    )
    execution_time_ms: int = Field(
        ..., ge=0, description="Total execution time in milliseconds"
    )
