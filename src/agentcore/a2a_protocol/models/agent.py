...
class AgentEndpoint(BaseModel):
    ...
    protocols: List[str] = Field(default=["jsonrpc-2.0"], description="Supported protocols")
    description: Optional[str] = Field(None, description="Endpoint description")
    health_check_path: Optional[str] = Field(default="/health", description="Health check path")
...
class AgentAuthentication(BaseModel):
    ...
    @classmethod
    def validate_auth_config(cls, v: Dict[str, Any], values: Dict[str, Any]) -> Dict[str, Any]:
...
class AgentCapability(BaseModel):
    ...
    def validate_capability_name(cls, v: str) -> str:
        ...
        if not v.replace("_", "").replace("-", "").replace(".", "").isalnum():
            raise ValueError("Capability name must contain only alphanumeric characters, hyphens, underscores, and dots")
        return v


class AgentRequirements(BaseModel):
    """Agent operational requirements."""
    min_memory_mb: Optional[int] = Field(None, description="Minimum memory requirement in MB")
    min_cpu_cores: Optional[float] = Field(None, description="Minimum CPU cores")
    required_capabilities: List[str] = Field(default_factory=list, description="Required capabilities from other agents")
    supported_languages: List[str] = Field(default_factory=list, description="Supported programming languages")
    runtime_environment: Optional[str] = Field(None, description="Required runtime environment")
...
class AgentMetadata(BaseModel):
    ...
    source_code_url: Optional[HttpUrl] = Field(None, description="Source code URL")
...
class AgentCard(BaseModel):
    ...
    agent_id: str = Field(default_factory=lambda: str(uuid4()), description="Unique agent identifier")
    ...
    endpoints: List[AgentEndpoint] = Field(..., min_length=1, description="Agent communication endpoints")

    # Capabilities and requirements
    capabilities: List[AgentCapability] = Field(default_factory=list, description="Agent capabilities")
    requirements: Optional[AgentRequirements] = Field(None, description="Agent requirements")

    # Security and authentication
    authentication: AgentAuthentication = Field(..., description="Authentication configuration")
    ...
    def get_primary_endpoint(self) -> Optional[AgentEndpoint]:
        ...
        for endpoint_type in [EndpointType.HTTPS, EndpointType.HTTP, EndpointType.WEBSOCKET_SECURE, EndpointType.WEBSOCKET]:
...
class AgentRegistrationRequest(BaseModel):
    ...
    override_existing: bool = Field(default=False, description="Whether to override existing registration")
...
class AgentDiscoveryQuery(BaseModel):
    ...
    status: Optional[AgentStatus] = Field(None, description="Agent status filter")
    tags: Optional[List[str]] = Field(None, description="Agent tags filter")
    category: Optional[str] = Field(None, description="Agent category filter")
    name_pattern: Optional[str] = Field(None, description="Agent name pattern (regex)")
