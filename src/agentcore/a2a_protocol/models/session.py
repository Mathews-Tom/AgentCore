...
class SessionContext(BaseModel):
    ...
    variables: dict[str, Any] = Field(
        default_factory=dict, description="Session-scoped variables"
    )
    agent_states: dict[str, dict[str, Any]] = Field(
        default_factory=dict, description="Per-agent state"
    )
    shared_memory: dict[str, Any] = Field(
        default_factory=dict, description="Shared memory across agents"
    )
    execution_history: list[dict[str, Any]] = Field(
        default_factory=list, description="Execution event history"
    )
...
class SessionSnapshot(BaseModel):
    ...
    participant_agents: list[str] = Field(
        default_factory=list, description="Participating agent IDs"
    )

    # Context and state
    context: SessionContext = Field(
        default_factory=SessionContext, description="Session context"
    )

    # Associated resources
    task_ids: list[str] = Field(default_factory=list, description="Associated task IDs")
    artifact_ids: list[str] = Field(
        default_factory=list, description="Associated artifact IDs"
    )
    ...
    timeout_seconds: int = Field(
        default=3600, ge=60, le=86400, description="Session timeout (60s-24h)"
    )
    max_idle_seconds: int = Field(
        default=300, ge=30, le=3600, description="Max idle time (30s-1h)"
    )
...
class SessionCreateRequest(BaseModel):
    ...
    timeout_seconds: int = Field(
        default=3600, ge=60, le=86400, description="Session timeout"
    )
    max_idle_seconds: int = Field(
        default=300, ge=30, le=3600, description="Max idle time"
    )
    tags: list[str] = Field(default_factory=list, description="Session tags")
...
class SessionQuery(BaseModel):
    ...
    state: SessionState | None = Field(None, description="Filter by state")
    owner_agent: str | None = Field(None, description="Filter by owner agent")
    participant_agent: str | None = Field(
        None, description="Filter by participant agent"
    )
    priority: SessionPriority | None = Field(None, description="Filter by priority")
