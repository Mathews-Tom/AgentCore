...
class SignedRequest(BaseModel):
    ...
    nonce: str = Field(default_factory=lambda: str(uuid4()), description="Request nonce")
...
class RateLimitInfo(BaseModel):
    ...
    window_start: datetime = Field(default_factory=datetime.utcnow, description="Rate limit window start")
    window_duration_seconds: int = Field(default=60, description="Rate limit window duration")
...
class AuthenticationRequest(BaseModel):
    ...
    agent_id: str = Field(..., description="Agent identifier")
    credentials: Dict[str, Any] = Field(..., description="Credentials (e.g., API key, certificate)")
    requested_permissions: Optional[List[Permission]] = Field(None, description="Requested permissions")
