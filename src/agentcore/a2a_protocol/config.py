...
class Settings(BaseSettings):
    ...
    MAX_CONCURRENT_CONNECTIONS: int = Field(default=1000, description="Max WebSocket connections")
    MESSAGE_TIMEOUT_SECONDS: int = Field(default=30, description="Message timeout in seconds")
    AGENT_DISCOVERY_TTL: int = Field(default=300, description="Agent discovery TTL in seconds")
    TASK_EXECUTION_TIMEOUT: int = Field(default=3600, description="Task execution timeout in seconds")
    ...
    POSTGRES_DB: str = Field(default="agentcore", description="PostgreSQL database name")
    ...
    REDIS_CLUSTER_URLS: List[str] = Field(
        default=[],
        description="Redis cluster URLs (overrides REDIS_URL if provided)"
    )
