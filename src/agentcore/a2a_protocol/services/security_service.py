...
class SecurityService:
    ...
    def register_public_key(self, agent_id: str, public_key_pem: str) -> bool:
        ...
        try:
            ...
            self.logger.error("Failed to register public key", agent_id=agent_id, error=str(e))
            return False

    def sign_request(self, agent_id: str, payload: Dict[str, Any]) -> Optional[SignedRequest]:
        """
    ...
    def verify_signature(self, signed_request: SignedRequest) -> bool:
        ...
        if not public_key:
            self.logger.warning("No public key for agent", agent_id=signed_request.agent_id)
        ...
        try:
            ...
            if signed_request.is_expired():
                self.logger.warning("Signed request expired", agent_id=signed_request.agent_id)
                return False

            # Check for replay attack (nonce reuse)
            if signed_request.nonce in self._used_nonces[signed_request.agent_id]:
                self.logger.warning("Replay attack detected", agent_id=signed_request.agent_id, nonce=signed_request.nonce)
            ...
            self.logger.error("Signature verification failed", agent_id=signed_request.agent_id, error=str(e))
            self._security_stats["invalid_signatures"] += 1
            return False
    ...
    def get_rate_limit_info(self, agent_id: str) -> Optional[RateLimitInfo]:
    ...
    def reset_rate_limit(self, agent_id: str) -> None:
        ...
        if agent_id in self._rate_limits:
            ...
            self._rate_limits[agent_id].window_start = datetime.utcnow()
    ...
    def authenticate_agent(self, request: AuthenticationRequest) -> AuthenticationResponse:
        """
