...
class HealthMonitor:
    ...
    async def start(self) -> None:
        ...
        self._running = True
        self._http_client = AsyncClient(timeout=self.health_check_timeout)
    ...
    async def check_all_agents(self) -> dict[str, bool]:
        ...
        async with get_session() as session:
            ...
            agents = [
                a
                for a in agents
                if a.status in (AgentStatus.ACTIVE, AgentStatus.MAINTENANCE)
            ]
    ...
    async def check_agent_health(
        self, agent_id: str, endpoint: str | None = None
    ) -> bool:
        """
        ...
        if is_healthy:
            ...
            total_successful = (
                self._stats["total_checks"] - self._stats["checks_failed"]
            )
        ...
        async with get_session() as session:
            ...
            if self._consecutive_failures[agent_id] >= self.failure_threshold:
                await AgentRepository.update_status(
                    session, agent_id, AgentStatus.ERROR
                )
                logger.warning(
                ...
                if agent and agent.status == AgentStatus.ERROR:
                    await AgentRepository.update_status(
                        session, agent_id, AgentStatus.ACTIVE
                    )
    ...
    async def cleanup_old_metrics(self, days_to_keep: int = 7) -> int:
        ...
        async with get_session() as session:
            rows_deleted = await HealthMetricRepository.cleanup_old_metrics(
